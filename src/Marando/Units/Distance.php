...
class Distance
{
    ...
    /**
     * Creates a distance instance from a number of astronomical units.
     * Optionally the definition of the astronomical unit can be overridden.
     *
     ...
     * the velocity of the speed of light in a vacuum and the number of days in
     * a year can be overridden.
     *
    ...
    public function __toString()
    {
        ...
        if (($unit == null && $this->pc > 1) || $unit == 'pc') {
            ...
            if (($unit == null && $this->au > 1) || $unit == 'au') {
                $value = $this->au;
                $units = 'AU';
            } else {
                if (($unit == null && $this->km > 1) || $unit == 'km') {
                    $value = $this->km;
                    $units = 'km';
                } else {
                    if (($unit == null && $this->mi > 1) || $unit == 'mi') {
                        $value = $this->mi;
                        $units = 'mi';
                    } else {
                        if (($unit == null && $this->m > 1) || $unit == 'm') {
                            $value = $this->m;
                            $units = 'm';
                        } else {
                            if (($unit == null && $this->cm > 1) || $unit == 'cm') {
                                $value = $this->cm;
                                $units = 'cm';
                            } else {
                                if (($unit == null && $this->mm > 1) || $unit == 'mm') {
                                    $value = $this->mm;
                                    $units = 'mm';
                                }
                            }
...
/*
// work like in others, store it and use in to string
//have default too
$distance->format('%1.4f', 'au');
sprintf first, units second
